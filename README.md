...
- **在线阅读**：支持左右滚动和上下滚动。可以切换自动吸附与无缝模式，手机端可以双指缩放当前图片。
...
阅读方向、自动吸附、无缝模式和懒加载范围会保存在浏览器中。信息栏位置目前固定在底部。

手机端可以将当前图片双指缩放至 1 至 4 倍，并在放大后单指拖动。双指缩回 1 倍、放大后双击或点击顶栏的重置按钮可恢复原尺寸。翻页、切换章节或更改阅读模式时也会重置缩放；关闭自动吸附时，重置后会回到缩放前的滚动位置。

## 已知限制

- 搜索、作品信息和未缓存的图片依赖上游服务。PWA 不能让这些请求完全离线运行。
- 部分网络环境可能无法访问图片 CDN。
